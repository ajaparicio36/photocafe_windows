...
class FrameOne extends ConsumerWidget {
  ...
  Future<Uint8List> _generatePdf(
    ...
    // Load the 2x2 frame background
    final frameImageBytes = await rootBundle.load(
      'assets/frames/2by2_frame1.png',
    );
    ...
    // For 2x2 mode, we only use 2 photos
    final int photoCount = 2;
    final double topOffset = 50; // Adjust based on 2x2 frame design
    final double photoWidth = 140;
    final double photoHeight = 250; // 9:16 aspect ratio
          ...
          return pw.Stack(
            children: [
              ...
              for (int i = 0; i < photoCount; i++)
                pw.Positioned(
                  left: 20,
                  top: topOffset + i * 180, // Vertical spacing for 2 photos
                  child: pw.Container(
                    width: photoWidth,
                    height: photoHeight,
              ...
              for (int i = 0; i < photoCount; i++)
                pw.Positioned(
                  left: 180, // Right side positioning
                  top: topOffset + i * 180,
                  child: pw.Container(
                    width: photoWidth,
                    height: photoHeight,
